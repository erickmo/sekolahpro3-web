...
| AKA-FEAT-06 | Satu Pintu Akademik Fase 1 — Kelas (7 rute), Jadwal (13 rute), Ekskul (8 rute), PPDB (11 rute) dipindah ke bawah `/akademik`; sidebar 21→16 pintu; stub redirect di rute lama; workspace nav grup (Ringkasan/Pengaturan/Penilaian/Kegiatan); hub `?go=` deep-link; kartu Tautan Modul di dashboard workspace | Feature | Done (2026-06-11) | `akademikNav.test.ts`, `akademik-layout.test.ts`, `jadwalExport.test.ts` + 1408 vitest hijau total |
| AKA-FEAT-07 | Satu Pintu Akademik Fase 2 (IA-only FE) — `siswa/rombel` → stub ke `/akademik/$ta/kelas/anggota` (duplikat); `siswa/pendaftaran` (3 rute) → halaman workspace `/akademik/$ta/pendaftaran`; `absensi` (5 rute) → submodule `/akademik/$ta/absensi` (chrome-bypass, sumber TA dari `$ta`); pintu sidebar absensi dihapus; grup pill-nav Kehadiran + Penerimaan; stub redirect di semua rute lama | Feature | Done (2026-06-17) | `akademik-fase2.test.ts`, `akademikNav.test.ts`, `akademik.ta.dashboard.test.tsx` + 1453 vitest hijau total |
