...
import { SITUS_PAGE_GUIDES, type SitusGuideId } from "../pageGuides";
import { SCHOOL_ROLE_LABEL } from "../../../lib/schoolGuideRole";

const EXPECTED_IDS: SitusGuideId[] = [
  "dashboard",
  "agenda",
  "berita",
  "domain",
...
describe("SITUS_PAGE_GUIDES", () => {
  it("contains every expected page id", () => {
    for (const id of EXPECTED_IDS) {
      expect(SITUS_PAGE_GUIDES[id]).toBeDefined();
    }
  });

  it("has no unexpected page ids", () => {
    expect(Object.keys(SITUS_PAGE_GUIDES).sort()).toEqual(
      [...EXPECTED_IDS].sort(),
    );
  });
  ...
  for (const id of EXPECTED_IDS) {
    describe(`entry "${id}"`, () => {
      const entry = SITUS_PAGE_GUIDES[id];
