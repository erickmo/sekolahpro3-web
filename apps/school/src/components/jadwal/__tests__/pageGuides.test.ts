...
const EXPECTED_IDS: JadwalGuideId[] = [
  "dashboard",
  "daftar",
  "override",
  "slot-override",
...
describe("JADWAL_PAGE_GUIDES", () => {
  it("contains every expected page id", () => {
    for (const id of EXPECTED_IDS) {
      expect(JADWAL_PAGE_GUIDES[id]).toBeDefined();
    }
  });

  it("has no unexpected page ids", () => {
    expect(Object.keys(JADWAL_PAGE_GUIDES).sort()).toEqual([...EXPECTED_IDS].sort());
  });

  for (const id of EXPECTED_IDS) {
    describe(`entry "${id}"`, () => {
      const entry = JADWAL_PAGE_GUIDES[id];
