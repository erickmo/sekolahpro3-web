...
type QuickAction = {
  to: "/sch/$sekolah/ppdb/calon-siswa" | "/sch/$sekolah/ppdb/seleksi" | "/sch/$sekolah/ppdb/pembayaran" | "/sch/$sekolah/ppdb/daftar-ulang" | "/sch/$sekolah/ppdb/gelombang" | "/sch/$sekolah/ppdb/pengaturan";
  label: string;
  description: string;
...
const PPDB_FLOW_STEPS: ModuleFlowStep[] = [
  { key: "tahun-ajaran", label: "Tahun Ajaran", hint: "Aktifkan TA berjalan", href: "/sch/$sekolah/akademik/tahun-ajaran" },
  { key: "gelombang", label: "Gelombang", hint: "Atur kuota & periode", href: "/sch/$sekolah/ppdb/gelombang" },
  { key: "pengaturan", label: "Pengaturan", hint: "Biaya, formulir, alur", href: "/sch/$sekolah/ppdb/pengaturan" },
  { key: "calon", label: "Calon Siswa", hint: "Terima pendaftaran", href: "/sch/$sekolah/ppdb/calon-siswa" },
  { key: "seleksi", label: "Seleksi", hint: "Jadwal & hasil", href: "/sch/$sekolah/ppdb/seleksi" },
  { key: "pembayaran", label: "Pembayaran", hint: "Verifikasi tagihan", href: "/sch/$sekolah/ppdb/pembayaran" },
  { key: "daftar-ulang", label: "Daftar Ulang", hint: "Konfirmasi diterima", href: "/sch/$sekolah/ppdb/daftar-ulang" },
];

const QUICK_ACTIONS: QuickAction[] = [
  { to: "/sch/$sekolah/ppdb/calon-siswa", label: "Calon Siswa", description: "Kelola data pendaftar baru", icon: <IconUsers /> },
  { to: "/sch/$sekolah/ppdb/seleksi", label: "Seleksi", description: "Jadwal & hasil seleksi", icon: <IconCheck /> },
  { to: "/sch/$sekolah/ppdb/pembayaran", label: "Pembayaran", description: "Verifikasi pembayaran PPDB", icon: <IconWallet /> },
  { to: "/sch/$sekolah/ppdb/daftar-ulang", label: "Daftar Ulang", description: "Konfirmasi peserta diterima", icon: <IconGrad /> },
  { to: "/sch/$sekolah/ppdb/gelombang", label: "Gelombang", description: "Atur gelombang & kuota", icon: <IconCalendar /> },
  { to: "/sch/$sekolah/ppdb/pengaturan", label: "Pengaturan", description: "Konfigurasi modul PPDB", icon: <IconSettings /> },
];
  ...
  const perluPerhatian = useMemo<AttentionItem[]>(() => {
    ...
    if (stats.pembayaranPendingCount > 0) {
      ...
      items.push({
        ...
        actionHref: firstPending ? `/ppdb/${firstPending.noPendaftaran}` : "/sch/$sekolah/ppdb/pembayaran",
      });
    }
    if (PPDB_DOKUMEN_KURANG_STUB > 0) {
      items.push({
        ...
        actionHref: "/sch/$sekolah/ppdb/calon-siswa",
      });
    }
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard
          ...
          actionHref="/sch/$sekolah/ppdb/gelombang"
        ...
        <StatCard
          ...
          accent="emerald"
          urgency="normal"
          actionHref="/sch/$sekolah/ppdb/seleksi"
          renderLink={(href, children) => <Link to={href}>{children}</Link>}
        />
        <StatCard
      ...
      <ModuleFlow
        ...
        renderLink={(href, children) => (
          <Link to={href as "/sch/$sekolah/ppdb/gelombang"} params={{ sekolah }}>
            {children}
          </Link>
        )}
      />
      ...
      >
        <AttentionList
          ...
          renderLink={(href, children) => <Link to={href as "/sch/$sekolah/ppdb/calon-siswa"} params={{ sekolah }}>{children}</Link>}
        />
      </SectionCard>
      ...
      <SectionCard
        ...
        padded={false}
        action={
          <Link to="/sch/$sekolah/ppdb/daftar" params={{ sekolah }} className="text-xs text-brand hover:underline">
            Lihat semua
          </Link>
        }
      >
      ...
      <p className="text-xs text-muted-fg">
        Tip: buka{" "}
        <Link to="/sch/$sekolah/ppdb/daftar" params={{ sekolah }} className="text-brand hover:underline">
          Pendaftaran
        </Link>{" "}
...
export const Route = createFileRoute("/sch/$sekolah/ppdb/")({ component: PpdbDashboard });
