...
const INFRA_FLOW_STEPS: ModuleFlowStep[] = [
  { key: "daftar", label: "Daftar Gedung", hint: "Registrasi gedung", href: "/sch/$sekolah/infrastruktur/daftar-gedung" },
  { key: "lantai", label: "Lantai", hint: "Definisi per lantai", href: "/sch/$sekolah/infrastruktur/lantai" },
  { key: "ruangan", label: "Ruangan", hint: "Setup ruang & kapasitas", href: "/sch/$sekolah/infrastruktur/ruangan" },
  { key: "utilitas", label: "Utilitas", hint: "Listrik, air, dll", href: "/sch/$sekolah/infrastruktur/utilitas" },
];
...
const AKSI_CEPAT: { to: string; label: string; desc: string; icon: React.ReactNode }[] = [
  { to: "/sch/$sekolah/infrastruktur/ruangan", label: "Ruangan", desc: "Kelola ruangan & kapasitas", icon: <IconHome /> },
  { to: "/sch/$sekolah/infrastruktur/lantai", label: "Lantai", desc: "Struktur lantai per gedung", icon: <IconGrad /> },
  { to: "/sch/$sekolah/infrastruktur/utilitas", label: "Utilitas", desc: "PLN, PDAM, internet, gas", icon: <IconBell /> },
];
  ...
  const perluPerhatian = useMemo<AttentionItem[]>(() => {
    ...
    if (stats.utilitasAnomali > 0) {
      items.push({
        ...
        actionHref: "/sch/$sekolah/infrastruktur/utilitas",
      });
    }
    return items;
    ...
    <div className="space-y-6">
      <PageHeader
        ...
        actions={
          <>
            <Link
              to="/sch/$sekolah/infrastruktur/daftar-gedung" params={{ sekolah }}
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard
          ...
          accent="brand"
          urgency="normal"
          actionHref="/sch/$sekolah/infrastruktur/ruangan"
      ...
      <ModuleFlow
        ...
        renderLink={(href, children) => (
          <Link to={href as "/sch/$sekolah/infrastruktur/daftar-gedung"} params={{ sekolah }}>
            {children}
          </Link>
        )}
      />
      ...
      >
        <AttentionList
          ...
          renderLink={(href, children) => (
            <Link to={href as "/sch/$sekolah/infrastruktur/utilitas"} params={{ sekolah }}>{children}</Link>
          )}
        />
      </SectionCard>
      ...
      <SectionCard
        ...
        action={
          <Link to="/sch/$sekolah/infrastruktur/ruangan" params={{ sekolah }} className="text-xs text-brand hover:underline">
            Lihat semua
          </Link>
        }
      >
            ...
            {ruanganTerbaru.map((r) => (
              <li key={r.name} className="flex items-center gap-3 py-3 first:pt-0 last:pb-0">
                ...
                <div className="min-w-0 flex-1">
                  <Link
                    to="/sch/$sekolah/infrastruktur/ruangan" params={{ sekolah }}
...
export const Route = createFileRoute("/sch/$sekolah/infrastruktur/")({ component: InfraDashboardPage });
