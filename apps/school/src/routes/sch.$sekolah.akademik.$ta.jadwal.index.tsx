...
const JADWAL_FLOW_STEPS: ModuleFlowStep[] = [
  { key: "slot", label: "Definisi Slot", hint: "Jam pelajaran dasar", href: "/sch/$sekolah/akademik/$ta/jadwal/slot" },
  { key: "daftar", label: "Susun Jadwal", hint: "Mapping mapel-guru-kelas", href: "/sch/$sekolah/akademik/$ta/jadwal/daftar" },
  { key: "override", label: "Override", hint: "Libur & hari khusus", href: "/sch/$sekolah/akademik/$ta/jadwal/override" },
  { key: "slot-override", label: "Slot Override", hint: "Penyesuaian per hari", href: "/sch/$sekolah/akademik/$ta/jadwal/slot-override" },
];

const QUICK_LINKS: { to: string; label: string; description: string }[] = [
  { to: "/sch/$sekolah/akademik/$ta/jadwal/daftar", label: "Jadwal Pelajaran", description: "Lihat grid & daftar mingguan" },
  { to: "/sch/$sekolah/akademik/$ta/jadwal/slot", label: "Slot Jadwal", description: "Definisi jam pelajaran" },
  { to: "/sch/$sekolah/akademik/$ta/jadwal/override", label: "Jadwal Override", description: "Libur & hari khusus" },
  { to: "/sch/$sekolah/akademik/$ta/jadwal/slot-override", label: "Slot Override", description: "Penyesuaian slot per hari" },
];
  ...
  const perluPerhatianItems = useMemo<AttentionItem[]>(() => {
    ...
    if (STUB_KONFLIK_SLOT > 0) {
      items.push({
        ...
        actionHref: "/sch/$sekolah/akademik/$ta/jadwal/daftar",
      });
    }
    if (STUB_OVERRIDE_AKTIF > 0) {
      items.push({
        ...
        actionHref: "/sch/$sekolah/akademik/$ta/jadwal/override",
      });
    }
    if (STUB_GURU_IZIN_DAMPAK > 0) {
      items.push({
        ...
        actionHref: "/sch/$sekolah/akademik/$ta/jadwal/slot",
      });
    }
    return items;
  }, []);
    ...
    <div className="space-y-6">
      ...
      <ModuleFlow
        ...
        renderLink={(href, children) => (
          <Link to={href as "/sch/$sekolah/akademik/$ta/jadwal/slot"} params={{ sekolah, ta }}>
            {children}
          </Link>
        )}
      />
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        <SectionCard
          ...
          padded={false}
          action={
            <Link to="/sch/$sekolah/akademik/$ta/jadwal/daftar" params={{ sekolah, ta }} className="text-xs font-medium text-brand hover:underline">
              Lihat semua
            </Link>
          }
        >
...
export const Route = createFileRoute("/sch/$sekolah/akademik/$ta/jadwal/")({ component: JadwalDashboardPage });
