...
type Row = { name: string; nama_ruangan: string; jenis?: string; lantai?: string; kapasitas?: number; status?: string };

const COLUMNS: Column<Row>[] = [
  { key: "name", header: "ID", sortable: true, cell: (r) => <span className="font-mono text-xs">{r.name}</span> },
  { key: "nama_ruangan", header: "Nama", sortable: true, cell: (r) => r.nama_ruangan },
...
function RuanganPage() {
  return (
    <ResourceListPage<Row>
      ...
      title="Ruangan"
      doctype="Ruangan"
      fields={["name", "nama_ruangan", "jenis", "lantai", "kapasitas", "status"]}
      rowKey={(r) => r.name}
      columns={COLUMNS}
      defaultSort={{ key: "nama_ruangan", dir: "asc" }}
      searchFields={["name", "nama_ruangan"]}
      selectFilters={[
