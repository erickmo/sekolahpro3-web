...
function PembayaranListPage() {
  ...
  const cols: Column<PaymentEntry>[] = [
    { key: "name", header: "No.", cell: (r) => (
        <Link to="/$sekolah/akuntansi/buku-besar/pembayaran/$name" params={{ sekolah, name: r.name }} className="font-mono text-xs text-brand hover:underline">{r.name}</Link>
      ), width: "180px" },
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        ...
        actions={
          <Link to="/$sekolah/akuntansi/buku-besar/pembayaran/new" params={{ sekolah }}>
...
export const Route = createFileRoute("/$sekolah/akuntansi/buku-besar/pembayaran/")({
