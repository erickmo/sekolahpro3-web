...
const ABSENSI_FLOW_STEPS: ModuleFlowStep[] = [
  { key: "jadwal", label: "Pastikan Jadwal", hint: "Slot pelajaran aktif", href: "/sch/$sekolah/akademik/$ta/jadwal/daftar" },
  { key: "harian", label: "Absensi Harian", hint: "Wali kelas catat siswa", href: "/sch/$sekolah/akademik/$ta/absensi/daftar" },
  { key: "pelajaran", label: "Absensi Pelajaran", hint: "Guru mapel per slot", href: "/sch/$sekolah/akademik/$ta/absensi/pelajaran" },
  { key: "guru", label: "Absensi Guru", hint: "Kehadiran tenaga pendidik", href: "/sch/$sekolah/akademik/$ta/absensi/guru" },
];
...
function AbsensiDashboardPage() {
  const { sekolah, ta } = useParams({ from: "/sch/$sekolah/akademik/$ta/absensi/" });
  ...
  const perluPerhatianItems = useMemo<AttentionItem[]>(() => {
    ...
    for (const { row, persen, total, hadir } of kelasIssues) {
      items.push({
        ...
        actionHref: "/sch/$sekolah/akademik/$ta/absensi/pelajaran",
      });
    }
    ...
    if (ringkasan.siswaAlpaBeruntun > 0) {
      items.push({
        ...
        actionHref: "/sch/$sekolah/siswa/wali",
      });
    }
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <StatCard
          ...
          accent="rose"
          urgency="critical"
          actionHref="/sch/$sekolah/akademik/$ta/absensi/daftar"
          renderLink={renderStatLink}
        />
        <StatCard
      ...
      <ModuleFlow
        ...
        renderLink={(href, children) => (
          <Link to={href as "/sch/$sekolah/akademik/$ta/absensi/daftar"} params={{ sekolah, ta }}>
            {children}
          </Link>
        )}
      />
      ...
      <SectionCard title="Aksi Cepat" description="Buka modul absensi yang sering digunakan.">
        <div className="grid gap-3 sm:grid-cols-3">
          <Link to="/sch/$sekolah/akademik/$ta/absensi/pelajaran" params={{ sekolah, ta }}
          ...
          <Link to="/sch/$sekolah/akademik/$ta/absensi/guru" params={{ sekolah, ta }}
            className="group rounded-lg border border-border bg-card p-4 hover:border-brand hover:bg-brand/5 transition-colors">
            <div className="flex items-center gap-3">
          ...
          <Link to="/sch/$sekolah/akademik/$ta/absensi/daftar" params={{ sekolah, ta }}
            className="group rounded-lg border border-border bg-card p-4 hover:border-brand hover:bg-brand/5 transition-colors">
            <div className="flex items-center gap-3">
...
export const Route = createFileRoute("/sch/$sekolah/akademik/$ta/absensi/")({ component: AbsensiDashboardPage });
