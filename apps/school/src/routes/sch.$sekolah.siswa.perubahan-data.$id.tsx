...
function PerubahanDetailPage() {
  ...
  const { id } = useParams({ from: "/sch/$sekolah/siswa/perubahan-data/$id" });
  ...
  if (docQuery.isError || !doc) {
    return (
      <div className="p-6">
        ...
        <div className="mt-4">
          <Link to="/sch/$sekolah/siswa/perubahan-data" params={{ sekolah }} className="text-brand hover:underline">
            ← Kembali ke daftar
          </Link>
        </div>
      </div>
...
export const Route = createFileRoute("/sch/$sekolah/siswa/perubahan-data/$id")({ component: PerubahanDetailPage });
