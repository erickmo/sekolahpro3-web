...
function AkadDetailPage() {
  ...
  const cols: Column<JadwalRow>[] = [
    ...
    { key: "name", header: "ID Jadwal", cell: (r) => (
      <Link to="/kop/$sekolah/angsuran/$name" params={{ sekolah, name: r.name }} className="font-mono text-xs text-brand hover:underline">
        {r.name}
      </Link>
    ) },
...
export const Route = createFileRoute("/kop/$sekolah/pembiayaan/$name")({
