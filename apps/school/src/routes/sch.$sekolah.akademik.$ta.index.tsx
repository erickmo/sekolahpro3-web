...
const AKADEMIK_FLOW_STEPS: ModuleFlowStep[] = [
  ...
  { key: "asesmen", label: "Input Nilai Test", hint: "Nilai per test/ulangan", href: "/sch/$sekolah/akademik/$ta/asesmen" },
  { key: "entri", label: "Entri Nilai", hint: "Input nilai siswa", href: "/sch/$sekolah/akademik/$ta/entri-nilai" },
  { key: "raport", label: "Raport", hint: "Susun & cetak raport", href: "/sch/$sekolah/akademik/$ta/raport" },
];
...
const QUICK_ACTION_GROUPS: QuickActionGroup[] = [
  {
    ...
    actions: [
      { to: "/sch/$sekolah/akademik/$ta/asesmen", label: "Input Nilai Test", description: "Input nilai satu test untuk satu kelas, cepat.", icon: <IconEdit className="h-4 w-4 shrink-0" />, accent: "brand" },
      { to: "/sch/$sekolah/akademik/$ta/entri-nilai", label: "Entri Nilai", description: "Rekap nilai per siswa × komponen.", icon: <IconChart className="h-4 w-4 shrink-0" />, accent: "sky" },
    ],
  },
  {
    role: "admin",
  ...
  },
  {
    ...
    actions: [
      { to: "/sch/$sekolah/akademik/$ta/raport", label: "Raport", description: "Susun & cetak raport siswa.", icon: <IconFile className="h-4 w-4 shrink-0" />, accent: "emerald" },
      { to: "/sch/$sekolah/akademik/$ta/entri-nilai", label: "Monitoring Nilai", description: "Pantau kelengkapan nilai per kelas.", icon: <IconChart className="h-4 w-4 shrink-0" />, accent: "sky" },
    ],
  },
];
...
/**
 * Link params for the dashboard's quick-links. Akademik feature pages now live
 * under the `$ta` segment, so links into them must carry the active `ta`. Master
 * pages ignore the extra `ta` at runtime (only params named in the route path are
 * substituted), so returning both keeps a single shape that type-satisfies every
 * cast `to` target — akademik (`{sekolah,ta}`) and master (`{sekolah}`) alike.
 */
function linkParamsFor(sekolah: string, ta: string): { sekolah: string; ta: string } {
  return { sekolah, ta };
}
...
function AkademikDashboardPage() {
  ...
  const renderStatLink = (href: string, children: React.ReactNode) => (
    <Link to={href as "/sch/$sekolah/master/kkm"} params={linkParamsFor(sekolah, ta)}>
      {children}
    </Link>
  );
  ...
  const perluPerhatianItems = useMemo<AttentionItem[]>(() => {
    ...
    if (cutOffDays !== null && cutOffDays <= CUTOFF_WARN_DAYS) {
      items.push({
        ...
        actionHref: "/sch/$sekolah/akademik/$ta/entri-nilai",
      });
    }
    ...
    <div className="space-y-6">
      ...
      <ModuleFlow
        ...
        renderLink={(href, children) => (
          <Link to={href as "/sch/$sekolah/master/kurikulum"} params={linkParamsFor(sekolah, ta)}>
            {children}
          </Link>
        )}
      />
          ...
          {anyLoading ? (
            ...
            <AttentionList
              ...
              renderLink={(href, children, className) => (
                <Link
                  to={href as "/sch/$sekolah/akademik/$ta/entri-nilai"}
                  params={linkParamsFor(sekolah, ta)}
                  className={className}
                >
                  {children}
                </Link>
      ...
      <div className="space-y-2">
        {group.actions.map((a) => (
          <Link
            ...
            to={a.to as "/sch/$sekolah/akademik/$ta/asesmen"}
            params={linkParamsFor(sekolah, ta)}
...
export const Route = createFileRoute("/sch/$sekolah/akademik/$ta/")({ component: AkademikDashboardPage });
