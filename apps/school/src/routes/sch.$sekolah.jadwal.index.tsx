...
const JADWAL_FLOW_STEPS: ModuleFlowStep[] = [
  { key: "slot", label: "Definisi Slot", hint: "Jam pelajaran dasar", href: "/sch/$sekolah/jadwal/slot" },
  { key: "daftar", label: "Susun Jadwal", hint: "Mapping mapel-guru-kelas", href: "/sch/$sekolah/jadwal/daftar" },
  { key: "override", label: "Override", hint: "Libur & hari khusus", href: "/sch/$sekolah/jadwal/override" },
  { key: "slot-override", label: "Slot Override", hint: "Penyesuaian per hari", href: "/sch/$sekolah/jadwal/slot-override" },
];

const QUICK_LINKS: { to: string; label: string; description: string }[] = [
  { to: "/sch/$sekolah/jadwal/daftar", label: "Jadwal Pelajaran", description: "Lihat grid & daftar mingguan" },
  { to: "/sch/$sekolah/jadwal/slot", label: "Slot Jadwal", description: "Definisi jam pelajaran" },
  { to: "/sch/$sekolah/jadwal/override", label: "Jadwal Override", description: "Libur & hari khusus" },
  { to: "/sch/$sekolah/jadwal/slot-override", label: "Slot Override", description: "Penyesuaian slot per hari" },
];
  ...
  const perluPerhatianItems = useMemo<AttentionItem[]>(() => {
    ...
    if (STUB_OVERRIDE_AKTIF > 0) {
      items.push({
        ...
        actionHref: "/sch/$sekolah/jadwal/override",
      });
    }
    ...
    <div className="space-y-6">
      ...
      <ModuleFlow
        ...
        renderLink={(href, children) => (
          <Link to={href as "/sch/$sekolah/jadwal/slot"} params={{ sekolah }}>
            {children}
          </Link>
        )}
      />
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        <SectionCard
          ...
          padded={false}
          action={
            <Link to="/sch/$sekolah/jadwal/daftar" params={{ sekolah }} className="text-xs font-medium text-brand hover:underline">
              Lihat semua
            </Link>
          }
        >
...
export const Route = createFileRoute("/sch/$sekolah/jadwal/")({ component: JadwalDashboardPage });
