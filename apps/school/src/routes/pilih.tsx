    ...
    selectKoperasi.mutate(
      { name: koperasiName },
      {
        onSuccess: (resp) => {
          ...
          // tree /sch. Bookmarkable, tak terikat harus pilih sekolah dulu.
          navigate({ to: "/kop/$sekolah", params: { sekolah: resp.slug } });
        },
      },
    );
  };
    ...
    select.mutate(
      { name: only.sekolah },
      {
        onSuccess: (resp) => {
          ...
          navigate({ to: "/sch/$sekolah", params: { sekolah: resp.slug } });
        },
      },
    );
 ...
 * existing per-sekolah koperasi dashboard (/kop/$sekolah).
 */
...
/**
 * Footer for /pilih. Renders server-managed content from SekolahPro
...
export const Route = createFileRoute("/pilih")({
