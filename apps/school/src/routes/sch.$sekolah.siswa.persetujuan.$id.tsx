...
function ConsentDetailPage() {
  ...
  const { id } = useParams({ from: "/sch/$sekolah/siswa/persetujuan/$id" });
  ...
  if (docQuery.isError || !docQuery.data) {
    return (
      <div className="p-6">
        ...
        <div className="mt-4">
          <Link to="/sch/$sekolah/siswa/persetujuan" params={{ sekolah }} className="text-brand hover:underline">
            ← Kembali ke daftar
          </Link>
        </div>
      </div>
...
export const Route = createFileRoute("/sch/$sekolah/siswa/persetujuan/$id")({ component: ConsentDetailPage });
