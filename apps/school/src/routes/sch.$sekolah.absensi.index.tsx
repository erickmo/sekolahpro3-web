...
const ABSENSI_FLOW_STEPS: ModuleFlowStep[] = [
  { key: "jadwal", label: "Pastikan Jadwal", hint: "Slot pelajaran aktif", href: "/sch/$sekolah/jadwal/daftar" },
  { key: "harian", label: "Absensi Harian", hint: "Wali kelas catat siswa", href: "/sch/$sekolah/absensi/daftar" },
  { key: "pelajaran", label: "Absensi Pelajaran", hint: "Guru mapel per slot", href: "/sch/$sekolah/absensi/pelajaran" },
  ...
  const perluPerhatianItems = useMemo<AttentionItem[]>(() => {
    ...
    for (const { row, persen, total, hadir } of kelasIssues) {
      items.push({
        ...
        actionHref: "/sch/$sekolah/absensi/pelajaran",
      });
    }
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <StatCard
          ...
          accent="rose"
          urgency="critical"
          actionHref="/sch/$sekolah/absensi/daftar"
          renderLink={renderStatLink}
        />
        <StatCard
      ...
      <ModuleFlow
        ...
        renderLink={(href, children) => (
          <Link to={href as "/sch/$sekolah/absensi/daftar"} params={{ sekolah }}>
            {children}
          </Link>
        )}
      />
      ...
      <SectionCard title="Aksi Cepat" description="Buka modul absensi yang sering digunakan.">
        <div className="grid gap-3 sm:grid-cols-3">
          <Link to="/sch/$sekolah/absensi/pelajaran" params={{ sekolah }}
          ...
          <Link to="/sch/$sekolah/absensi/guru" params={{ sekolah }}
            className="group rounded-lg border border-border bg-card p-4 hover:border-brand hover:bg-brand/5 transition-colors">
            <div className="flex items-center gap-3">
          ...
          <Link to="/sch/$sekolah/absensi/daftar" params={{ sekolah }}
            className="group rounded-lg border border-border bg-card p-4 hover:border-brand hover:bg-brand/5 transition-colors">
            <div className="flex items-center gap-3">
...
export const Route = createFileRoute("/sch/$sekolah/absensi/")({ component: AbsensiDashboardPage });
