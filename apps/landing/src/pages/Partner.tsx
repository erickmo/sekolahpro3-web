...
import {
  ...
  type PartnerOrg,
  type CaseStudy,
  type PartnerHero as PartnerHeroData,
  type PartnerSchoolsSection,
  type PartnerCaseSection,
  type PartnerOrgsSection,
  type PartnerCTAContent,
} from "../lib/partner";

export function Partner() {
  const { hero, schools_section, case_section, orgs_section, cta, schools, orgs, case_studies } =
    usePartnerContent();
  return (
    <>
      <PartnerHero hero={hero} />
      <SchoolGrid section={schools_section} schools={schools} />
      <CaseStudies section={case_section} items={case_studies} />
      <OrgLogos section={orgs_section} orgs={orgs} />
      <PartnerCTA cta={cta} />
    </>
  );
}

function PartnerHero({ hero }: { hero: PartnerHeroData }) {
  return (
    ...
    <section className="py-16 sm:py-20">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex items-end justify-between mb-8 flex-wrap gap-4">
          <h2 className="text-2xl sm:text-3xl font-semibold text-fg">{section.heading}</h2>
          <p className="text-sm text-muted-fg">{section.lead}</p>
        </div>
...
function CaseStudies({
  ...
  section: PartnerCaseSection;
  items: CaseStudy[];
}) {
  return (
    <section className="py-16 sm:py-20 border-y border-border bg-muted/40">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <p className="text-sm font-medium text-brand">{section.eyebrow}</p>
        <h2 className="mt-2 text-3xl sm:text-4xl font-semibold text-fg max-w-2xl">
          {section.title_main}{" "}
          <em className="not-italic font-serif italic text-brand">{section.title_italic}</em>
        </h2>
...
function OrgLogos({
  ...
  return (
    <section className="py-16 sm:py-20">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <h2 className="text-2xl sm:text-3xl font-semibold text-fg">{section.heading}</h2>
        <p className="mt-2 text-muted-fg max-w-2xl">{section.lead}</p>
...
function PartnerCTA({ cta }: { cta: PartnerCTAContent }) {
  return (
